...
as 
--SELECT t.name,f.name, bi.count, f.price, f.price*bi.count AS totalPrice 
--FROM dbo.BillInfo AS bi, dbo.Bill AS b, dbo.Food AS f, dbo.TableFood as t
--WHERE bi.idBill = b.id AND bi.idFood = f.id AND b.status = 0 AND b.idTable = 29 and t.id = b.idTable

SELECT b.idTable,t.name as[TableName],f.name as [FoodName], bi.count, f.price as [price], f.price*bi.count AS [totalPrice], b.totalPrice as[PriceCheckout], b.discount
FROM dbo.BillInfo AS bi, dbo.Bill AS b, dbo.Food AS f, dbo.TableFood as t
WHERE bi.idBill = b.id AND bi.idFood = f.id AND b.status = 0 and t.id = b.idTable


select * from viewbill where TableName = N'Bàn 12'
